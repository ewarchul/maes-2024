ma_es_msr <- function(par, fn, ..., lower, upper, control = list()) {
  label <- "ma_es_msr"
  norm <- function(x) {
  ...
  while (counteval < budget) {
    ...
    previous_kth_point = prev_arfitness[j_msr]
    K_succ = length(which(arfitness < previous_kth_point))
    z_stat <- (2 / lambda) * (K_succ - ((lambda + 1) / 2))
    s <- (1 - c_msr) * s + c_msr * z_stat
    sigma <- sigma * exp(s / damps)
