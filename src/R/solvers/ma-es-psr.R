ma_es_psr <- function(par, fn, ..., lower, upper, control = list()) {
  label <- "ma_es_psr"
  norm <- function(x) {
  ...
  while (iter < maxiter) {
    ...
    r <- order(c(arfitness, prev_arfitness)) - 1
    z_psr <- sum(rr[r < lambda] - rr[r >= lambda])
    z_psr <- z_psr/(lambda^2) - p_target

    s <- (1 - c_psr) * s + c_psr * z_psr
