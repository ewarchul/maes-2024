...
def run_benchmark(benchmark_spec: Path):
    args = shlex.split(
        f'R --no-save --slave -e "library(cecb); cecb::run_benchmark(\\"{benchmark_spec}\\")"'
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="cec_runner", description="starts CEC benchmark using yaml specification"
    )
